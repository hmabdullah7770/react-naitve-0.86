...
import MediaThumbnail from '../hook/Mediathumbnail'; // adjust path
import useVideoModeration from '../hook/useVideoModeration'; // adjust path

const InlineImageGrid = React.memo(
  ({onClose, onMediaChange, onVideoSettingsOpen, onDataChange}) => {
    ...
    // Notify parent component when media data changes
    useEffect(() => {
      if (onDataChange) {
        const mediaData = {
          selectedMedia,
        ...
        onDataChange(mediaData);
      }
    }, [selectedMedia, selectedLayout, videoSettings, onDataChange]);
        ...
        launchImageLibrary(options, response => {
          ...
          try {
            ...
            const newMedia = validAssets.map(asset => {
              const isVideo = asset.type?.includes('video') || false;
              return {
                uri: asset.uri,
                type: asset.type,
                ...
                // Video-only fields: filled in once native frame extraction
                // and backend moderation resolve
                posterUri: null,
                thumbnailExtracting: isVideo,
                thumbnailError: false,
                moderationStatus: isVideo ? 'pending' : null, // pending | checking | approved | flagged | error
            ...
            // Extract frames + run nudity moderation for every newly added
            // video, instantly. Thumbnail appears as soon as extraction
            // finishes; moderationStatus updates separately once the
            // backend/Vision API call resolves.
            moderateNewMedia(newMedia, selectedMedia.length);
    ...
    const handleRemoveMedia = index => {
      try {
        // If the removed item is a video, clean up any frame files that
        // were extracted for it (poster + any not-yet-uploaded frames)
        // so they don't linger in cache after the video is gone.
        const removedMedia = selectedMedia[index];
        cleanupMediaFrames(removedMedia);
      ...
      Alert.alert(
        ...
        [
          ...
          },
          {
            ...
            style: 'destructive',
            onPress: () => {
              // Clean up frame files for every video before clearing state
              selectedMedia.forEach(media => cleanupMediaFrames(media));
          ...
          items.push(
            <View
              ...
              <MediaThumbnail
                ...
                imageStyle={styles.mediaImage}
                onImageError={() => {
                  console.error('Image load error in two-column');
                  handleRemoveMedia(i);
                }}
                VideoThumbnailOverlay={VideoThumbnailOverlay}
                onVideoPress={handleVideoClick}
    ...
    // Method to get current media data for external access
    const getMediaData = () => {
      return {
        selectedMedia,
        selectedLayout,
...
// import ThumbnailBottomnav from './ThumbnailBottomnav';
// import LayoutOptionStyles from './LayoutOptionStyles';
// import useVideoFrameExtraction from '../hook/useVideoFrameExtraction'; // adjust path
// import MediaThumbnail from '../hook/Mediathumbnail'; // adjust path
// import useVideoModeration from '../hook/useVideoModeration';
...
//     // Extracts a poster frame for each video right after it's uploaded
//     const {extractFramesForNewMedia} = useVideoFrameExtraction(
//       setSelectedMedia,
//     );
...
//                 width: asset.width || 0,
//                 height: asset.height || 0,
//                 // Video-only fields: filled in once native frame extraction resolves
//                 posterUri: null,
//                 thumbnailExtracting: isVideo,
//                 thumbnailError: false,
//               };
...
//             // Extract a poster frame for every newly added video, instantly
//             extractFramesForNewMedia(newMedia, selectedMedia.length);
...
// verrrrrrrrry olllllllllllllllllllllllllllllld
